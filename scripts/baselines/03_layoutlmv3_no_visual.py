...
box broadcast, and word-to-subword label alignment (-100 for
continuations) in one call -- no manual alignment code needed.
...
def build_examples(images: list[dict], tokenizer) -> list[dict]:
    ...
    for img in images:
        ...
        encoding = tokenizer(
            text=words, boxes=boxes, word_labels=word_label_ids, truncation=True, max_length=MAX_LENGTH
        )
        examples.append({
            "id": img["id"],
